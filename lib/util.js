...
export function cookieHeader(name, value, { maxAge = 60 * 60 * 24 * 30, clear = false, secure = false } = {}) {
  const base = clear
...
function sessionSecret() {
  return process.env.KWOTA_SECRET || "kwota-demo-share-secret";
}

export function signSession(businessId) {
  const payload = Buffer.from(JSON.stringify({ bid: businessId, iat: Date.now() })).toString(
    "base64url"
  );
...
export function readSession(token) {
  ...
  try {
    ...
    if (!data.bid || Date.now() - Number(data.iat) > 30 * 86400000) return null;
    return data.bid;
  } catch {
    return null;
  }
