...
function buildPrompt({ text, trade, imageCount, sheetCount }) {
  return `You are quoting for a South African ${trade || "trades"} business.
Read the voice/text, every photo (nameplates, site pics, WhatsApp screenshots, handwritten notes), AND any Excel/CSV dump.
...
- notes: timing, exclusions, extras, what you saw in photos / sheets.
- ${imageCount} photo(s) attached. ${sheetCount || 0} spreadsheet(s) attached (rows are listed in the notes).
- Prefer line items from the spreadsheet when they have descriptions and prices. Voice/photos fill gaps and client/site.

Voice / notes:
"""${String(text || "").slice(0, 6000)}"""`;
}

export async function grokParse({ text, images = [], trade = "trades", sheetCount = 0 }, key) {
...
export async function parseQuoteInput({ text, images, sheets, trade, defaultDeposit } = {}) {
  const { extractSheet, decodeSheetPayload } = await import("./sheet.js");
  ...
  for (const raw of sheets || []) {
    const decoded = decodeSheetPayload(raw);
    if (!decoded) continue;
    try {
      const out = extractSheet(decoded.buf, decoded.name);
      if (out.text) sheetBits.push(out.text);
      sheetItems.push(...out.items);
      if (!sheetClient && out.clientName) sheetClient = out.clientName;
      if (!sheetSite && out.site) sheetSite = out.site;
      if (!sheetPhone && out.clientPhone) sheetPhone = out.clientPhone;
    } catch (e) {
      sheetBits.push(`(Could not read ${decoded.name}: ${e.message})`);
    }
  }

  const combined = [String(text || "").trim(), ...sheetBits].filter(Boolean).join("\n\n");
  const key = process.env.XAI_API_KEY;
  const hasInput = combined || imageCount;

  const mergeSheet = (parsed) => {
    if (!parsed.clientName && sheetClient) parsed.clientName = sheetClient;
    if (!parsed.site && sheetSite) parsed.site = sheetSite;
    if (!parsed.clientPhone && sheetPhone) parsed.clientPhone = sheetPhone;
    const aiPriced = (parsed.items || []).filter((i) => i.unitPrice > 0).length;
    if (sheetItems.length && aiPriced < Math.max(1, Math.floor(sheetItems.length * 0.4))) {
      parsed.items = sheetItems;
      parsed.warnings = parsed.warnings || [];
      parsed.warnings.push({
        severity: "info",
        message: `Used ${sheetItems.length} line(s) from the spreadsheet. Check them.`,
      });
    } else if (sheetItems.length) {
      parsed.warnings = parsed.warnings || [];
      parsed.warnings.push({
        severity: "info",
        message: `Spreadsheet ${sheetItems.length} row(s) was in the scan.`,
      });
    }
    return parsed;
  };
  ...
  if (key && hasInput) {
    try {
      const ai = await grokParse(
        { text: combined, images, trade, sheetCount: sheetBits.length },
        key
      );
      return {
        source: "ai",
        ...addChecks(mergeSheet(ai), { defaultDeposit, imageCount }),
      };
    } catch (e) {
      const fb = heuristicParse(combined);
      if (sheetItems.length) fb.items = sheetItems;
      return {
        source: sheetItems.length ? "sheet" : "heuristic",
        error: e.message,
        ...addChecks(mergeSheet(fb), { defaultDeposit, imageCount }),
      };
    }
  }
